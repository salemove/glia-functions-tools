...
async function getAuthorizationToken(apiKeyId, apiKeySecret) {
  ...
  async function fetchMessageHistory(env, engagementId) {
    // fetch data from env variables
    const apiKeyId = env.GLIA_USER_API_KEY;
    const apiKeySecret = env.GLIA_USER_SECRET;
  ...
  async function fetchTransferType(env, engagementId) {
    const apiKeyId = env.GLIA_USER_API_KEY;
    const apiKeySecret = env.GLIA_USER_SECRET;
    const token = await getAuthorizationToken(
      apiKeyId,
  ...
  async function onInvoke(request, env) {
      ...
      const AI_API_KEY = parsed.engine_settings.AI_API_KEY;
      const AI_ENDPOINT = parsed.engine_settings.AI_ENDPOINT;
      const AI_MODEL = parsed.engine_settings.AI_MODEL;
      const AI_PERSONALITY = parsed.engine_settings.AI_PERSONALITY;
      ...
      const message_history = await fetchMessageHistory(env, parsed.engagement_id);
      ...
      if(responseText.includes("DONE")) {
        ...
        const transferType = await fetchTransferType(env, parsed.engagement_id);
