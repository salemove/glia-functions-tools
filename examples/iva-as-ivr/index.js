...
exports.handler = async function(event, context) {
    try {
        ...
        const utterance = onlyDigits(requestBody.utterance);
        const engagementId = requestBody.engagement_id;
        const messageId = requestBody.message_id;

        const bearer = await createBearerToken(process.env.GLIA_KEY_ID, process.env.GLIA_KEY_SECRET);
        ...
        const updatedVisitor = await updateVisitor(bearer, visitor.id, process.env.GLIA_SITE_ID, customAttributesResult);
        ...
        switch (intentState) {
            ...
            case '3':
                ...
                break
            case '4':
                return returnSuggestionToVisitor(event.body)
            case '44':
                // redact engagement data
                const redactionRequestBody = {
                    site_id: process.env.GLIA_SITE_ID,
            ...
            case '11':
                return transferWidget(process.env.GLIA_TRANSFER_QUEUE_ID);
            case '12':
                await callbackWidget(bearer, engagementId, visitor);
                break
            case '9':
                console.log('invoking transferWidget');
                return transferWidget(process.env.GLIA_TRANSFER_QUEUE_ID, visitor)
            default:
                await transferWidget(process.env.GLIA_TRANSFER_QUEUE_ID)
        }
    } catch(error) {
        console.log(error)
