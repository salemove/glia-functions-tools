/**
 * Creates the HTML for the development server test page
 * 
 * @param {number} port - Server port number
 * @returns {string} - HTML content
 */
export default function createTestHtml(port) {
  return `<!DOCTYPE html>
<html>
<head>
  <title>Glia Functions Development Server</title>
  <style>
...
<body>
  ...
  <script>
    // Simple helper to make development easier
    function $(id) { return document.getElementById(id); }
    
    // Store server port from Node.js
    ...
    function renderHistoryItem(item) {
      ...
      el.addEventListener('click', () => {
        ...
        $('payload').value = JSON.stringify(item.payload, null, 2);
